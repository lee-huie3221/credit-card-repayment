...
st.pyplot(fig2)
st.caption("💡 🟢 绿色圆点：全额还款 | 🟠 橙色方块：分期还款 | 🔴 红色三角：最低还款")

st.markdown("---")

# ========== 大学生常见消费场景（100%匹配文档5个故事） ==========
st.subheader("📋 大学生常见消费场景参考")

reference_scenes = pd.DataFrame({
    "消费场景": ["新款手机", "报班升学", "求职/实习全套装备", "美妆护肤", "健身私教"],
    "消费本金(元)": [5000, 20000, 8000, 3000, 15000],
    "12期分期利息(元)": [420, 1680, 672, 252, 1260],
    "12期最低还款利息(元)": [978, 3912, 1565, 587, 2934],
    "分期总还款(元)": [5420, 21680, 8672, 3252, 16260],
    "最低总还款(元)": [5978, 23912, 9565, 3587, 17934]
})
...
st.markdown("""
> 📌 **数据说明（严格匹配文档）**：
> - 最低还款：日利率0.05%，按月复利，每月还10%，剩余金额持续计息12个月
> - 账单分期：12期平均月费率0.7%，总利息=本金×0.7%×12
""")
